    ...
    * the ElevenLabs synthesis — a char-equivalent (``len(tts_input)``) summed into
    ...
    try:
        ...
        if audio_type == "summary" and sync_client:
            ...
            tts_input = llm_result.choices[0].message.content or content_text
        # audio_type == "podcast": no LLM script step here — podcast script
        # generation is owned by ai_service.py (POD-2's chunk_text / podcast
        # helper). This route synthesizes ``content_text`` directly for
        # podcast, unchanged from prior behavior, until that helper lands.
